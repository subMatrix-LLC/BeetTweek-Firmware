...
//				float flywheelEngagefrictionFactor = 0.001f;
			...
			if(driveStrengthFiltered > 0.0f)
				driveAngle = MathExtras::WrapMax(float(rotorFluxAngle + M_PI_2 + phaseOffsetExtra), float(M_TWO_PI));
			else
				driveAngle = MathExtras::WrapMax(float(rotorFluxAngle - M_PI_2 + phaseOffsetExtra), float(M_TWO_PI));
			...
			//space vector modulation should be more power efficient than simple pwm.  but for now simple PWM with center aligned is working well.

			//SetMotorPWMDrivesSpaceVector(&motorDevA,  fabs(driveStrength), driveAngle);
			SetMotorPWMDrives(&motorDevA, MotorDriveWaveForm_Sinusoidal,  fabs(driveStrengthFiltered), float(motorDevice->direction)*driveAngle);
